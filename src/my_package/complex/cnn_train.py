...
x_phrase, x_context, y, vocabulary, vocabulary_inv, r = input_data.load_datas(domain_dir, pickle_dir, 1, 2)
...
x_dev_phrase = x_dev_phrase[:10000]
x_dev_context = x_dev_context[:10000]
y_dev = y_dev[:10000]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def dev_step(x_phrase_batch, x_context_batch, y_batch, writer=None):
            """
            ...
            step, summaries, loss, precision, recall, f1_score = sess.run(
                [global_step, dev_summary_op, cnn.loss, cnn.precision, cnn.recall, cnn.f1_score],
                feed_dict)
            time_str = datetime.datetime.now().isoformat()
            print("{}: step {}, loss {:g}, f1 score {:g}".format(time_str, step, loss, f1_score))
        ...
        for batch in batches:
            ...
            if current_step % FLAGS.evaluate_every == 0:
                print("\nEvaluation:")
                dev_step(x_dev_phrase, x_dev_context, y_dev, writer=dev_summary_writer)
                print("")
