...
class CNN(object):
    ...
    def __init__(self,
        ...
        with tf.device('/cpu:0'):
            ...
            while i < len(self.inputs):
                with tf.variable_scope("embedding-%d" % k):
                    ...
                    if k == 1:
                        self.embed = embeded_position_tile
                    if i == len(self.inputs) - 2:
                        self.dep_embedded = tf.concat(2, [word_embedded,
                                                          postag_embedded,
                                                          tf.tile(self.embeded_position, [1, lengths[i], 1])])
                k += 1
                i += 2
        # Create a convolution + maxpool layer for each filter size
        pooled_outputs = []
        for k in range(len(self.embedded_expanded)-1):
        ...
        with tf.name_scope("attention"):
            ...
            att_feature_raw = tf.reshape(trans_feature, [-1, 5])
            W_att = tf.Variable(
                tf.random_uniform([5, 1], -1.0, 1.0),
                name="W_att")
        ...
        with tf.name_scope("lstm"):
            ...
            self.outputs = self.BiLSTM(self.dep_embedded, lengths[11], embedding_size*3, seq_len)
        ...
        with tf.name_scope("output"):
            ...
            self.predictions = tf.cast(tf.greater(self.scores_proba, 0.5), tf.int64)
