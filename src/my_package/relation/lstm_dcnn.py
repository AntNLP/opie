    ...
    def __init__(self,
        ...
        with tf.device('/cpu:0'):
            ...
            self.word_embedding = tf.Variable(
                    tf.random_uniform([word_vocab_size, embedding_size], -1.0, 1.0), name="W1")
            W2 = tf.Variable(
        ...
        with tf.name_scope("attention"):
            # LSTM+OBT
            num_part = 3
            trans_feature = tf.pack(self.h_pool_flat[1:4])
            #  trans_feature = tf.pack(self.h_pool_flat)
            self.embed = trans_feature
