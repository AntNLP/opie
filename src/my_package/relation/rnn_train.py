...
tf.flags.DEFINE_integer("n_hidden", 64, "Dimensionality of hidden (default: 64)")
...
data = data_helper.load_datas(domain_dir, 1, 2)
save_pickle_file("rnn.%s.pickle" % domain, data)
data = load_pickle_file("rnn.%s.pickle" % domain)
...
save_pickle_file("rnn.%s.vocab_embeddings.pickle" % domain, vocab_embeddings)
vocab_embeddings = load_pickle_file("rnn.%s.vocab_embeddings.pickle" % domain)
...
print("middle length: ", lengths[3])
print("right length: ", lengths[5])
print("min length: ", lengths[7])
print("max length: ", lengths[9])
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        sess.run(rnn.word_embedding.assign(vocab_embeddings))
        ...
        def train_step(X_batch, y_batch):
            ...
            """
            feed_dict = {
              rnn.input0: np.array([e[0] for e in X_batch]),
              rnn.input1: np.array([e[1] for e in X_batch]),
              rnn.input2: np.array([e[2] for e in X_batch]),
              rnn.input3: np.array([e[3] for e in X_batch]),
              rnn.input4: np.array([e[4] for e in X_batch]),
              rnn.input5: np.array([e[5] for e in X_batch]),
              rnn.input6: np.array([e[6] for e in X_batch]),
              rnn.input7: np.array([e[7] for e in X_batch]),
              rnn.input8: np.array([e[8] for e in X_batch]),
              rnn.input9: np.array([e[9] for e in X_batch]),
              rnn.input10: np.array([e[10] for e in X_batch]),
              rnn.input11: np.array([e[11] for e in X_batch]),
              rnn.input12: np.array([e[12] for e in X_batch]),
              rnn.input13: np.array([e[13] for e in X_batch]),
              rnn.input_y: y_batch,
              rnn.dropout_keep_prob: FLAGS.dropout_keep_prob
            }
            #  for e in X_batch[0]:
                #  print(e)
            #  embed = sess.run(rnn.embed, feed_dict)
            #  print(embed)
            _, step, summaries, loss, f1_score = sess.run(
                [train_op, global_step, train_summary_op, rnn.loss, rnn.f1_score],
                feed_dict)
        ...
        def dev_step(X_batch, y_batch, writer=None):
            ...
            step, summaries, loss, pred = sess.run(
                [global_step, dev_summary_op, rnn.loss, rnn.pred],
                feed_dict)
            time_str = datetime.datetime.now().isoformat()
