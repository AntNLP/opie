...
# ==================================================
# Domain
tf.flags.DEFINE_string("domain", "reviews_Cell_Phones_and_Accessories", "domain name (default: 'reviews_Cell_Phones_and_Accessories')")
...
y_train, y_dev = y[:-test_num], y[-test_num:]
print("Vocabulary Size: {:d}".format(len(vocab_processor.vocabulary_)))
print("Train/Dev split: {:d}/{:d}".format(len(y_train), len(y_dev)))
print("Max Document length: {0}".format(max_document_length))
print("Min Document length: {0}".format(min_document_length))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        out_dir = os.path.abspath(os.path.join(multi_opin_expr_dir, "runs", timestamp))
        print("Writing to {}\n".format(out_dir))
        ...
        if FLAGS.embedding_init:
            embeddings = load_pickle_file(os.path.join(multi_opin_expr_dir,
                                                    "word2vec",
                                                    "embeddings.pickle"))
            vocab_embeddings = np.array(embeddings)
            sess.run(cnn.embeddings.assign(vocab_embeddings))
